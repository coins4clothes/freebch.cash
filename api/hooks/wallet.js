...
module.exports = function WalletHook(sails) {
  return {
    test: async function() {
      let sendMoneyResults;
      try {
        sendMoneyResults = await sails.hooks.wallet.sendMoney({ to: sails.config.wallet.paymentAddress })
      }
      catch(nope) {
        console.log('ERROR:', nope);
      }
    },
    sendMoney: async function(options) {
      let paymentAmount = 11000;

      // Ensure there are no undefined or duplicate
      // addresses in the from array.
      options.from = sails.hooks.wallet.BITBOX.Address.toCashAddress(sails.config.wallet.paymentAddress);

      let arrayOfInputAddresses = [];
      arrayOfInputAddresses.push({cashAddress: options.from});

      let toAddress = sails.hooks.wallet.BITBOX.Address.toCashAddress(options.to);
      ...
      while (arrayOfInputAddresses.length) {
        ...
        try {
          someResults = await sails.hooks.wallet.BITBOX.Address.utxo(justAddresses);
        }
        catch(nope) {
      ...
      while (userUtxos.length) {
        ...
        txData.feesNeeded = sails.hooks.wallet.BITBOX.BitcoinCash.getByteCount({ P2PKH: numOfInputs }, { P2PKH: 2 });
      ...
      let transactionBuilder = new sails.hooks.wallet.BITBOX.TransactionBuilder('bitcoincash');
      ...
      console.log('\tAdding output:', sails.config.wallet.paymentAddress, txData.changeNeeded);

      // Change address
      transactionBuilder.addOutput(sails.config.wallet.paymentAddress, txData.changeNeeded);
      ...
      try {
        ...
        for (let oneTxInput of txData.spendables) {
          ...
          try {
            inputSigningPair = sails.hooks.wallet.BITBOX.ECPair.fromWIF(sails.config.wallet.privateKey);
          }
          catch(nope) {
      ...
      try {
        submissionResults = await sails.hooks.wallet.BITBOX.RawTransactions.sendRawTransaction(hexTx);
      }
      catch (nope) {
    ...
    initialize: async function (done) {
      ...
      let doSetup = () => {
        sails.hooks.wallet.BITBOX = new BITBOXSDK();
      };
      ...
      sails.after(['hook:orm:loaded'], () => {
        ...
        // Send the signal that the wallet hook has been loaded;
        sails.emit('hook:wallet:ready');
